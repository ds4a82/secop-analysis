...
gerenciaUI <- function(id){
  ...
  insight <- radioButtons(
    ...
    inputId = ns("insight")
    , label = h4("Métrica")
    , choices = c("Venta Pond", "Costo Pond", "Utilidad Pond", "Cantidad")
    , selected = "Venta Pond"
    # , inline = TRUE
  )
  bar <- radioButtons(
    ...
    inputId = ns("bar")
    , label = h4("Periodo")
    , choices = c("Semanal", "Mensual", "Trimestral", "Anual")
    , selected = c("Trimestral")
    # , selectize = TRUE
    # , inline = TRUE
  )
  perspective <- radioButtons(
    ...
    , choices = c("Estado", "Clientes", "Vendedores"
                  # , "Unidad"
                  , "Tipo_Oportunidad")
    , selected = c("Clientes")
    # , inline = TRUE
  ...
  main <- list(""
     , fluidRow(column(width = 9, adds$inicio), column(width = 3, adds$filters))
     ...
     , fluidRow(""
      , crm_filtrosUI(ns("crm_filtros"), estados = c("Close", "Forecast", "Won"))
      , column(width = 4
     ...
     , fluidRow(
       box(plotlyOutput(ns("circle"))
           ...
           , title = "Oportunidades", footer = NULL, width = 6, height = NULL, status = "primary")
       , box(plotlyOutput(ns("bar_chart"))
             , box(bar, title= NULL, footer = NULL, width = 12, height = NULL, status = "primary")
             , title= "Facturación", footer = NULL, width = 6, height = NULL, status = "primary")
     )
        , fluidRow(
            tabBox(id = "gerencia_tabs"
                , title = 'Tablas'
                , tabPanel("Oportunidades"
                    , p('Estas oportunidades no se encuentran ponderadas:')
                    , DT::dataTableOutput(ns("oportunidades"))
                )
                , tabPanel("Clientes"
                    , p('Resumen de los clientes:')
                    , DT::dataTableOutput(ns("clientes"))
                )
                , tabPanel("Vendedores"
                    , p('Resumen de los vendedores:')
...
gerencia <- function(input, output, session){
  ...
  output$bar_chart <- renderPlotly({
    ...
    dummybarchart <- function(){
      ...
      fig <- fig %>% add_trace(x = ~Date, y = ~Wind, type = 'bar', name = 'Compras',
                               marker = list(color = '#C9EFF9'),
                               hoverinfo = "text",
                               text = ~paste(Wind, ' millones'))
      fig <- fig %>% add_trace(x = ~Date, y = ~Temp, type = 'scatter', mode = 'lines', name = 'Cantidad', yaxis = 'y2',
                               line = list(color = '#45171D'),
                               hoverinfo = "text",
                               text = ~paste(Temp, ' contratos'))
      fig <- fig %>% layout(title = '',
                            xaxis = list(title = ""),
                            yaxis = list(side = 'left', title = 'Compras', showgrid = FALSE, zeroline = FALSE),
                            yaxis2 = list(side = 'right', overlaying = "y", title = 'Cantidad', showgrid = FALSE, zeroline = FALSE))
